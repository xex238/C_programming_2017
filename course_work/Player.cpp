...
Player::Player(const String& string_for_load_car_texture, const String& string_for_load_arrows_texture, Map& map_1) // <- Стандартный конструктор
{
	x = map_1.Return_burn().x + 64 + 5;
	y = map_1.Return_burn().y + 128 + 5;
	vector = map_1.Return_vector();
	texture_cars.loadFromFile(string_for_load_car_texture);
...
void Player::Move_control() // <- Выбор движения машинки или обработка передвижения стрелочки
{
	if (arrow_is_move)
	{
...
void Player::Go_without_time(const int& vector, RenderWindow& window)
{
	this->vector = vector;
	switch (vector)
	{
...
void Player::Go_with_time(const int& vector)
{
	this->vector = vector;
	switch (vector)
	{
...
void Player::Player_MouseButtonPressed(const Event& event) // <- Обработка передвижения стрелочки
{
...
void Player::Player_MouseButtonReleased(const Event& event, const Map& map_1) // <- После отпускания кнопки мыши, которая тащила стрелочку
{
...
void Player::Restart_time() // <- Обновление времени
{
...
void Player::Update_pos(RenderWindow& window) // <- Обновление координат мыши
{
	pixel_pos = Mouse::getPosition(window);
	pos = window.mapPixelToCoords(pixel_pos);
}

void Player::Draw_arrows(RenderWindow& window) // <- Отрисовка стрелочек
{
...
void Player::Car_go(const Map& map_1, RenderWindow& window, Musics& my_music) // <- Автоматическое движение машинки к финишу
{
...
Sprite Player::Return_sprite_cars()
{
	return sprite_cars;
}

void Player::Go_to_vector_1(RenderWindow& window)
{
	for (int i = 0; i < 320; i++)
	{
		Go_without_time(1, window);
...
void Player::Restart_level(Map& map_1, Musics& my_music)
{
	size_arrows = -1;
...
bool Player::Return_enable_text_for_win()
{
	return enable_text_for_win;
}

Text Player::Return_text_for_win()
{
	return text_for_win;
}
