...
public:
	...
	void Move_control(const Vector2f& pos);
	...
	void Car_go(/*RenderWindow& window, */Musics& my_music);
	...
	bool Check_collision_with_arrows_for_car_go(const float& x, const float& y);

	bool Check_collision_with_exit_for_car_go(const float& x, const float& y);

	bool Check_on_true_vector(const float& x, const float& y);
...
private:
	...
	int burn_vector; // Вектор рождения
